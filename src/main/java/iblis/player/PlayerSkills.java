...
public class PlayerSkills {
	...
	public static final PlayerSkills SWORDSMANSHIP = new PlayerSkills("SWORDSMANSHIP",
			SharedIblisAttributes.SWORDSMANSHIP, 0.001f);
	...
	public static final PlayerSkills SHARPSHOOTING = new PlayerSkills("SHARPSHOOTING",
			SharedIblisAttributes.SHARPSHOOTING, 0.001f);
	public static final PlayerSkills ARMORSMITH = new PlayerSkills("ARMORSMITH", SharedIblisAttributes.ARMORSMITH,
			0.02f);
	public static final PlayerSkills WEAPONSMITH = new PlayerSkills("WEAPONSMITH", SharedIblisAttributes.WEAPONSMITH,
			0.02f);
	public static final PlayerSkills MECHANICS = new PlayerSkills("MECHANICS", SharedIblisAttributes.MECHANICS, 0.02f);
	public static final PlayerSkills MEDICAL_AID = new PlayerSkills("MEDICAL_AID", SharedIblisAttributes.MEDICAL_AID,
			0.02f);
	...
	public static final PlayerSkills EQUILIBRIUM = new PlayerSkills("EQUILIBRIUM", SharedIblisAttributes.EQUILIBRIUM,
			0.001f);
	...
	public void raiseSkill(EntityPlayer player, double d) {
		if (this.getCurrentValue(player) > cap)
			return;
		if (player.world.isRemote)
			throw new IllegalStateException("Skills should be raised only at server side.");
		int divider = 1;
		for (IAttribute iattribute = this.attribute; iattribute != SharedIblisAttributes.INTELLIGENCE; iattribute = iattribute
				.getParent()) {
	...
	public double getFullSkillValue(EntityPlayer entityLivingBase) {
		if (!enabled)
			return 0d;
		double value = 0d;
		for (IAttribute iattribute = this.attribute; iattribute != null; iattribute = iattribute.getParent()) {
